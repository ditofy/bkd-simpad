...
WHERE A.jns_surat='$kd_jns_surat' AND A.thn_pajak='$thn_pajak' AND A.bln_pajak='$bln_pajak' AND A.kd_obj_pajak='$kd_obj_pajak' AND A.no_urut_surat='$no_urut_surat'";
		...
		break;
	case "SPTPD":
		$query = "SELECT A.kd_kecamatan,A.kd_kelurahan,A.kd_obj_pajak,A.kd_keg_usaha,A.no_reg,A.kd_kecamatan||'.'||A.kd_kelurahan||'.'||A.kd_obj_pajak||'.'||A.kd_keg_usaha||'.'||A.no_reg AS nop,A.kd_provinsi||'.'||A.kd_kota||'.'||A.kd_jns||'.'||A.no_reg_wp AS npwpd,A.nm_usaha AS nm_objek,A.pokok_pajak,A.status_pembayaran,0 AS denda,B.nama,B.alamat,C.nm_obj_pajak,A.dasar_pengenaan_pajak as dasar,B.nik,A.bln_pajak||' - '||a.thn_pajak as masa FROM $schema.$table A INNER JOIN public.wp B ON B.kd_provinsi=A.kd_provinsi AND B.kd_kota=A.kd_kota AND B.kd_jns=A.kd_jns AND B.no_reg=A.no_reg_wp INNER JOIN public.obj_pajak C ON c.kd_obj_pajak=A.kd_obj_pajak
INNER JOIN public.keg_usaha D ON D.kd_obj_pajak=A.kd_obj_pajak and D.kd_keg_usaha=A.kd_keg_usaha		
WHERE A.jns_surat='$kd_jns_surat' AND A.thn_pajak='$thn_pajak' AND A.bln_pajak='$bln_pajak' AND A.kd_obj_pajak='$kd_obj_pajak' AND A.no_urut_surat='$no_urut_surat'";
		break;
		case "SSPD":
		$query = "SELECT A.kd_propinsi,A.kd_dati2,A.kd_kecamatan,A.kd_kelurahan,A.kd_blok,A.no_urut,A.kd_jns_op,A.kd_propinsi||'.'||A.kd_dati2||'.'||A.kd_kecamatan||'.'||A.kd_kelurahan||'.'||A.kd_blok||'.'||A.no_urut||'.'||A.kd_jns_op AS nop,B.kd_provinsi||'.'||B.kd_kota||'.'||B.kd_jns||'.'||B.no_reg AS npwpd,A.nama_sppt AS nm_objek,C.pokok_pajak_real as pokok_pajak,C.status_pembayaran,0 AS denda,B.nama,D.nm_obj_pajak,A.pengurangan,B.nik, B.alamat||'.'||B.kelurahan||'.'||B.kecamatan||'.'||B.kota AS alamat,C.NPOPKP as dasar,C.bln_pajak||' - '||C.thn_pajak as masa,C.kd_sh as kd_keg_usaha FROM bphtb.pelayanan A INNER JOIN public.wp B ON B.nik=A.nik INNER JOIN bphtb.sspd C ON C.tahun_p=A.tahun AND C.no_urut_p=A.no_urut_p  INNER JOIN public.obj_pajak D ON D.kd_obj_pajak=C.kd_obj_pajak
...
$status = array(
	...
	"kd_keg_usaha" => $data['kd_keg_usaha'],
	...
	"pajak" => number_format($data['pokok_pajak']),
	"masa_pajak" => $data['masa'],
	"dasar" => number_format($data['dasar']),
	"denda" => $data['denda'],
	"mtgkey" => $data['mtgkey'],
