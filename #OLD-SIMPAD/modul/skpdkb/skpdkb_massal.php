...
$thn_pajak_a = pg_escape_string($_GET['thn']);
$bln_pajak_a = pg_escape_string($_GET['bln']);
$jns_obj = pg_escape_string($_GET['jns']);
...
switch ($jns_obj) {
case "01":
	   $query = pg_query("select A.jns_surat||'.'||A.thn_pajak||'.'||A.bln_pajak||'.'||A.kd_obj_pajak||'.'||A.no_urut_surat AS no_surat,C.kd_provinsi||'.'||C.kd_kota||'.'||C.kd_jns||'.'||C.no_reg as npwpd,C.nama,b.nm_usaha as nama_objek,C.alamat,b.alamat_usaha as alamat_objek,A.pokok_pajak,B.kd_kecamatan||'.'||B.kd_kelurahan||'.'||B.kd_keg_usaha||'.'||B.no_reg as nop,b.thn_pajak,b.bln_pajak,D.nm_obj_pajak,D.perda,A.tgl_jatuh_tempo,A.dasar_pajak,A.pokok_pajak,A.pajak_disetor,A.pokok_pajak_baru,A.tgl_surat from air_tanah.skpdkb A INNER JOIN 
...
WHERE A.jns_surat='$jns_surat' and A.thn_pajak='$thn_pajak' and A.bln_pajak='$bln_pajak' and A.kd_obj_pajak='$kd_obj_pajak' and A.no_urut_surat='$no_urut_surat' ") or die('Query failed: ' . pg_last_error());
       break;
     /*  case "09":
	   $query = pg_query("select A.jns_surat||'.'||A.thn_pajak||'.'||A.bln_pajak||'.'||A.kd_obj_pajak||'.'||A.no_urut_surat AS no_surat,C.kd_provinsi||'.'||C.kd_kota||'.'||C.kd_jns||'.'||C.no_reg as npwpd,C.nama,b.nama_sppt as nama_objek,C.alamat,b.alamat_op as alamat_objek,A.pokok_pajak,B.kd_propinsi||'.'||B.kd_dati2||'.'||B.kd_kecamatan||'.'||B.kd_kelurahan||'.'||B.kd_blok||'.'||B.no_urut||'.'||B.kd_jns_op as nop,b.thn_pajak,b.bln_pajak,D.nm_obj_pajak,D.perda,A.tgl_jatuh_tempo,A.dasar_pajak,A.pokok_pajak,A.pajak_disetor,A.pokok_pajak_baru,A.tgl_surat from bphtb.skpdkb A INNER JOIN 
...
LEFT JOIN public.obj_pajak B on B.kd_obj_pajak=A.kd_obj_pajak WHERE A.thn_pajak='2018' and A.bln_pajak='12'") or die('Query failed: ' . pg_last_error());
       break;
...
<strong class="font10">MASA   : <?php echo strtoupper($bulan_s[$bln_pajak_a]); ?></strong>
<strong class="font10">TAHUN  : <?php echo $thn_pajak_a; ?></strong>
</pre>
	</td>
    <td align="center">
<pre class="font14">
