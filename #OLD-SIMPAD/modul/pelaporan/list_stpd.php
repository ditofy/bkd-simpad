...
if($kd_obj_pajak !== '02') {
$num_results = pg_query("SELECT COUNT(*) AS jumlah,SUM(A.pokok_pajak) AS g_total FROM $schema.stpd A INNER JOIN public.wp B ON B.kd_provinsi=A.kd_provinsi AND B.kd_kota=A.kd_kota AND B.kd_jns=A.kd_jns AND B.no_reg=A.no_reg_wp 
...
$(document).ready(function () {
	$('#cetak-list').click(function(){		
			...
			$.post( "modul/pelaporan/gen_pdf_list_stpd.php", { data: inputs, ukuran: $('#ukuran-kertas').val() })
  						.done(function( data1 ) {
	...
	$('#download-xls').click(function(){		
    		...
    		window.open('/print/list_stpd_xls.php');
			});	
	});
