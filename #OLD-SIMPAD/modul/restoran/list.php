...
$jns_penetapan = $arr_data['jenis-penetapan'];
if($kd_keg_usaha == ""){
...
$query = pg_query("SELECT A.kd_provinsi||'.'||A.kd_kota||'.'||A.kd_jns||'.'||A.no_reg_wp AS npwpd, A.kd_kecamatan||'.'||A.kd_kelurahan||'.'||A.kd_obj_pajak||'.'||A.kd_keg_usaha||'.'||A.no_reg AS nop,A.jenis_penetapan,CASE WHEN jenis_penetapan='01' THEN 'WARALABA' WHEN jenis_penetapan='02' THEN 'NON WARALABA' WHEN jenis_penetapan='03' THEN 'DATA LAMA' ELSE '' END AS jns_p,to_char(A.tgl_daftar, 'DD-MM-YYYY') AS tgl_daf,A.*,B.* FROM $schema.dat_obj_pajak A INNER JOIN public.wp B ON B.kd_provinsi=A.kd_provinsi AND B.kd_kota=A.kd_kota AND B.kd_jns=A.kd_jns AND B.no_reg=A.no_reg_wp WHERE A.kd_kecamatan LIKE '%$kd_kecamatan%' AND A.kd_kelurahan LIKE '%$kd_kelurahan%' AND A.kd_obj_pajak LIKE '%$kd_obj_pajak%' AND A.kd_keg_usaha LIKE '%$kd_keg_usaha%' AND A.no_reg LIKE '%$no_reg%' AND A.nm_usaha LIKE '%$nm_usaha%' AND A.kd_provinsi LIKE '%$kd_prov%' AND A.kd_kota LIKE '%$kd_kota%' AND A.kd_jns LIKE '%$kd_jns%' AND A.no_reg_wp LIKE '%$no_reg_wp%' AND B.nama LIKE '%$nm_wp%' AND A.jenis_penetapan LIKE '%$jns_penetapan%' AND status_pajak='1' ORDER BY A.id LIMIT $num_per_page OFFSET $offset") or die('Query failed: ' . pg_last_error());
?>
...
$(document).ready(function () {
	$('#cetak-list').click(function(){		
			...
			$.post( "modul/restoran/gen_pdf_list.php", { data: inputs, ukuran: $('#ukuran-kertas').val() })
  						.done(function( data1 ) {
	...
	$('#download-xls').click(function(){		
    		...
    		window.open('/print/list_restoran_xls.php');
			});	
	});
...
?>                                        
                                            <tr class="even pointer">
												...
												<td class=" "><?php echo $row['status_pajak']; ?></td>
