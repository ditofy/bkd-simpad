...
$sql = "SELECT A.*,B.nama,B.alamat,C.rek_apbd FROM restoran.skp A INNER JOIN public.wp B ON
...
<table width="100%" border="0" class="noborder" style="padding-left:5px;padding-top:5px;padding-right:5px;padding-bottom:5px;">
  <tr>
    <td colspan="12" align="justify"><?php 
			  if($row['bln_pajak'] == '01') { 
			  	echo "PAJAK RESTORAN ".$row['nm_usaha']." DI ".$row['alamat_usaha']." MASA PAJAK BULAN ".strtoupper($bulan[$row['bln_pajak']-1])." ".($row['thn_pajak']-1);
			  } else {
			  	echo "PAJAK RESTORAN ".$row['nm_usaha']." DI ".$row['alamat_usaha']." MASA PAJAK BULAN ".strtoupper($bulan[$row['bln_pajak']-1])." ".($row['thn_pajak']);
			  } 	
	?></td>
    </tr>
  <tr>
