...
$sql = "select * from restoran.dat_obj_pajak A where ( A.kd_kecamatan,A.kd_kelurahan,A.kd_obj_pajak,A.kd_keg_usaha,A.no_reg) NOT IN (SELECT  B.kd_kecamatan,B.kd_kelurahan,B.kd_obj_pajak,B.kd_keg_usaha,B.no_reg FROM restoran.sptpd B where B.thn_pajak='$thn_pajak' and B.bln_pajak='$bln_pajak') AND A.jenis_penetapan='02' and A.status_pajak='1'";
$tampil = pg_query($sql);
if($tampil) {
//Mempersiapkan
	for($x = 10; $x >= 1; $x--) {
...
//end denda */
		$query_cek = "SELECT * FROM restoran.sptpd WHERE kd_kecamatan='$row[kd_kecamatan]' and kd_kelurahan='$row[kd_kelurahan]' AND kd_obj_pajak='$row[kd_obj_pajak]' AND kd_keg_usaha='$row[kd_keg_usaha]' AND no_reg='$row[no_reg]' and thn_pajak='$thn_pajak' and bln_pajak='$bln_pajak'";
