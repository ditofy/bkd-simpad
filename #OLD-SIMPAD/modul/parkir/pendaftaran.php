...
$(document).ready(function () {
			...
			$('#proses-daftar-parkir').click(function(){
if($('#npwpd').val() == '' || $('#kd-kecamatan').val() == null || $('#kd-kelurahan').val() == null || $('#kd-keg-usaha').val() == null || $('#nama-usaha').val() == '' || $('#alamat-usaha').val() == '' || $('#tgl-daftar').val() == ''){
	...
	$("#proses-daftar-parkir").attr("disabled", "disabled");
	var inputs = $('#frm-daftar-parkir').serializeArray();
					$.post( "modul/parkir/p_tambah.php", { postdata: inputs })
  						.done(function( data1 ) {
   							notif('Notifikasi',data1,'success');
							$('#resetfrm').click();
							$("#proses-daftar-parkir").removeAttr('disabled');
  					});
}	
});
...
<form id="frm-daftar-parkir" name="frm-daftar-parkir" data-parsley-validate class="form-horizontal form-label-left">
	...
	<div class="form-group">
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
        	<input type="text" id="kd-obj-pajak" name="kd-obj-pajak" required="required" class="form-control col-md-7 col-xs-12" readonly="readonly" value="05 - Parkir">
        </div>
	</div>
	<div class="form-group">
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
			...
			<?php 
			$sql = "SELECT kd_keg_usaha||' - '||nm_keg_usaha AS value FROM public.keg_usaha WHERE kd_obj_pajak='05'";
			$tampil = pg_query($sql) or die('Query failed: ' . pg_last_error());
			while ($row = pg_fetch_array($tampil))
			{
	...
	<div class="form-group">
    	<div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
        	<button id="proses-daftar-parkir" type="button" class="btn btn-success">Simpan</button>
