...
$tnkb= strtoupper($arr_data['tnkb']);
$merk = strtoupper($arr_data['merk']);
$alamat = strtoupper($arr_data['alamat']);
$nm_wp = strtoupper($arr_data['nm-wp']);
$jenis_kd = strtoupper($arr_data['jenis-kd']);
$kd_kecamatan_l = $arr_data['kd-kecamatan'];
$kd_kelurahan_l = $arr_data['kd-kelurahan'];
if($kd_kecamatan_l == ""){
...
$num_results = pg_query("SELECT COUNT(*) AS jumlah FROM $schema.master A WHERE A.kd_kecamatan LIKE '%$kd_kecamatan_l%' AND A.kd_kelurahan LIKE '%$kd_kelurahan_l%' AND A.merek_tipe LIKE '%$merk%' AND A.tnkb LIKE '%$tnkb%' AND A.alamat LIKE '%$alamat%' AND A.nama LIKE '%$nm_wp%' AND A.jenis LIKE '%$jenis%'") or die('Query failed: ' . pg_last_error());
...
$query = pg_query("SELECT * from $schema.master A WHERE  A.kd_kecamatan LIKE '%$kd_kecamatan_l%' AND A.kd_kelurahan LIKE '%$kd_kelurahan_l%' AND A.merek_tipe LIKE '%$merk%' AND A.tnkb LIKE '%$tnkb%' AND A.alamat LIKE '%$alamat%' AND A.nama LIKE '%$nm_wp%' AND A.jenis LIKE '%$jenis%' ORDER BY A.tnkb LIMIT $num_per_page OFFSET $offset") or die('Query failed: ' . pg_last_error());
?>
...
<table id="t-list-restoran" class="table table-striped responsive-utilities jambo_table" style="font-size:9px;">
                                        <thead>
                                            <tr class="headings">                                         
                                                ...
                                                <th>MERK / TIPE </th>
												<th>JENIS </th>
												<th>TAHUN PEMBUATAN <?php echo $nm_wp; ?></th>
...
?>                                        
                                            <tr class="even pointer">
                                                ...
                                                <td class=" "><?php echo $row['tnkb']; ?></td>
												<td class=" "><?php echo $row['nik']; ?></td>
                                                <td class=" "><?php echo $row['nama']; ?></td>
                                                <td class=" "><?php echo $row['alamat']; ?></td>
												<td class=" "><?php echo $row['hp']; ?></td>
												<td class=" "><?php echo $row['merek_tipe']; ?></td>
												<td class=" "><?php echo $row['jenis']; ?></td>
												<td class=" "><?php echo $row['tahun_pembuatan']; ?></td>                                       
                                            </tr>
<?php
	}
}
