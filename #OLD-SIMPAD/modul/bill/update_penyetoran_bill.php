...
<script src="js/FungsiTerbilang.js"></script>
<script>
$(document).ready(function () {
	...
	$('#proses-update-setor-bill').click(function(){
	if( $('#nop').val() == '' || $('#npwpd').val() == '' || $('#no-bill').val() == '' ) {			
		...
		$.post( "modul/bill/p_update_pemungutan_bill_setor.php", { postdata: inputs })
  						.done(function( data1 ) {							
	...
	$("#no-bill").enterKey(function () {
		if( $('#no-bill').val() == '' ) {
			notif('Notifikasi','Isi No Bill','error');
				...
				$.post( "modul/bill/verifikasi_data_bill_update_setor.php", { seri: $('#seri-bill').val(), no_bill: $('#no-bill').val() })
  						.done(function( data1 ) {						
							var obj = jQuery.parseJSON(data1);
                                                        //console.log(obj);
							if( obj.status == "ok") {
								...
								$("#alamat-usaha").val(obj.alamat_usaha);
                                                                $("#bulan-pajak").val(obj.bln_pajak).trigger('change');                                                                
								$('#omset').removeAttr('readonly');
                                                                $("#omset").val(obj.dasar_pengenaan);
								$("#omset").focus();
                                                                $("#terbilang").html(terbilang($( "#omset" ).val()));
							} else {
								notif('Notifikasi',obj.msg,'error');
								$("#no-bill").removeAttr('readonly');																
							}
	...
	$("#omset").enterKey(function () {
		if( $("#omset").val != "") {
			$('#proses-update-setor-bill').click();
		} else {
			notif('Notifikasi','Isi Jumlah Omset','error');
		}
	});
	
        $( "#omset" ).keyup(function() {
            $("#terbilang").html(terbilang($( "#omset" ).val()));
        });
...
<div class="form-group">
	...
	<div class="form-group">
    	<div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
        	<button id="proses-update-setor-bill" type="button" class="btn btn-success">Update</button>
