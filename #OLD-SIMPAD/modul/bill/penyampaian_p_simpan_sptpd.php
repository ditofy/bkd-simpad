...
$pokok_pajak_real=round($pokok_pajak);
...
/*$query_cek = "SELECT * FROM $schema.sptpd WHERE kd_kecamatan='$kd_kecamatan' AND kd_kelurahan='$kd_kelurahan' AND kd_obj_pajak='$kd_obj_pajak' AND kd_keg_usaha='$kd_keg_usaha' AND no_reg='$no_reg' AND thn_pajak='$thn_pajak' AND bln_pajak='$bln_pajak'";
		...
		<?php	//} else  {*/
$query = "SELECT TO_NUMBER(MAX(no_urut_surat),'999999') AS no_max FROM $schema.sptpd WHERE thn_pajak='$thn_pajak' AND bln_pajak='$bln_pajak' AND kd_obj_pajak='$kd_obj_pajak'";
...
if($result) {
	...
	//$nops = "02.".$thn_pajak.".".$bln_pajak.".".$kd_obj_pajak.".".$no_urut_sptpd;
//deleteVaPjd($nops);
//$va = getVaPjd($nops, $nm_wp, $pokok_pajak);
//$va = 222222;
	pg_free_result($result);
	$query_input = "INSERT INTO $schema.sptpd(thn_pajak,bln_pajak,no_urut_surat,kd_kecamatan,kd_kelurahan,kd_obj_pajak,kd_keg_usaha,no_reg,kd_provinsi,kd_kota,kd_jns,no_reg_wp,nm_usaha,alamat_usaha,dasar_pengenaan_pajak,tarif,pokok_pajak,penyimpan,tgl_penyampaian,status_pembayaran,dat_pendukung,ket,qrcode) VALUES('$thn_pajak','$bln_pajak','$no_urut_sptpd','$kd_kecamatan','$kd_kelurahan','$kd_obj_pajak','$kd_keg_usaha','$no_reg','13','76','$kd_jns','$no_reg_wp','$nm_usaha','$alamat_usaha',$dasar_pengenaan_pajak,$tarif,$pokok_pajak_real,'$nip',TO_DATE('$tgl_penyampaian','DD-MM-YYYY'),'0','LAPORAN','$keterangan','$qrcode')";
