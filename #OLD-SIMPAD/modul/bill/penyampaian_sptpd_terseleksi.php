...
$(document).ready(function () {
	...
	$('#cetak-sptpd').click(function(){
		$('#modal-cnt').html("<center><br><br><img src='images/loading.gif' /><br>Loading<br><br></center>");		
			$.post( "modul/bill/gen_pdf_sptpd.php", {npwpd: $('#npwpd').val(),thn_pajak: $('#tahun-pajak').val(),bln_pajak: $('#bulan-pajak').val() })
  						.done(function( data1 ) {
	...
	$('#cetak-sspd').click(function(){
		$('#modal-cnt').html("<center><br><br><img src='images/loading.gif' /><br>Loading<br><br></center>");		
			$.post( "modul/bill/gen_pdf_sspd.php", { no_sspd: $('#no-sspd').val() })
  						.done(function( data1 ) {
							$('#modal-cnt').html("<object type='application/pdf' data='"+data1+"' width='100%' height='500'>this is not working as expected</object>");
  					});
	});
	...
	$('#proses-sptpd-restoran').click(function(){
		if( $('#nop').val() == '' || $('#npwpd').val() == '' || $('#tarif').val() == null || $('#dasar-pengenaan-pajak').val() == '' || $('#tanggal-penyampaian').val() == '' || $('#keterangan').val() == ''  ) {
			...
			var inputs = $('#frm-restoran-sptpd').serializeArray();
					$.post( "modul/bill/penyampaian_p_simpan_sptpd.php", { postdata: inputs })
  						.done(function( data1 ) {
							var obj = jQuery.parseJSON(data1);							
							if( obj.status == "ok") {
								notif('Notifikasi','Nomor VA Berhasil Digenerate di No : '+obj.no_sptpd,'success');
								$("#no-sptpd").val(obj.no_sptpd);
								$("#no-sspd").val(obj.no_sptpd);
								$("#cetak").removeAttr('disabled');								
							} 
							
							else if( obj.status == "sudah") {
								notif('Notifikasi','SPTPD ini sudah Dientrikan ','warning');
								$("#no-sptpd").val(obj.no_sptpd);
								$("#no-sspd").val(obj.no_sptpd);
								$("#cetak").removeAttr('disabled');								
							} 
	...
	$('#resetfrm').click(function(){
		...
		$("#cetak-sptpd").attr("disabled", "disabled");
		$("#cetak-sspd").attr("disabled", "disabled");
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>                                           
				<h4 class="modal-title" id="myModalLabel">SSPD</h4>
			</div>
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
    	<input type="text" id="no-sptpd" name="no-sptpd" class="form-control col-md-7 col-xs-12" placeholder="Nomor SSPD" readonly="readonly">
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
    	<select name="jenis-pajak" id="jenis-pajak" class="select2_single" style="width:200px">
			<option value="03"  selected="selected">PAJAK RESTORAN</option>			
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
		...
		<?php
			...
			$sql = "SELECT * FROM public.tarif WHERE kd_obj_pajak='03' ORDER BY kd_obj_pajak,kd_tarif DESC";
...
<div class="form-group">
	<label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Dasar Pengenaan Pajak</label>
    	<div class="col-md-3 col-sm-6 col-xs-12">
    		<input type="text" id="dasar-pengenaan-pajak" name="dasar-pengenaan-pajak" class="form-control col-md-7 col-xs-12" placeholder="Dasar Pengenaan Pajak" onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);">
    	</div>
</div>
<div class="form-group">
	<label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Keterangan</label>
    <div class="col-md-6 col-sm-6 col-xs-12">
    	<input type="text" id="keterangan" name="keterangan" class="form-control col-md-7 col-xs-12" placeholder="Isikan No Bill / Range No Bill Jika Pakai Bill" >
    </div>
</div>
<div class="form-group">
	...
	<div class="form-group">
    	<div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
			...
			<button id="cetak-sspd" type="button" class="btn btn-warning" data-toggle="modal" data-target=".bs-example-modal-lg">Cetak SSPD</button>	
