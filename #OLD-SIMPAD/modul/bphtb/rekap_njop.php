...
<button type="button" class="btn btn-success" id="tampilkan">Tampilkan Data NJOP</button>
</td>
</tr>
</table>
</form>
		<div class="col-md-12 col-sm-3 col-xs-12 bg-white" id="data-njop">		
...
function list_data(pg) {
	$('#data-njop').html("<center><br><br><img src='images/loading.gif' /><br>Loading<br><br></center>");	
	var inputs = $('#filter-data').serializeArray();	
			$.post( "modul/bphtb/cari_njop.php", { postdata: inputs, page: pg })
  						.done(function( data1 ) {
							$('#data-njop').html(data1);
  					});
};
		...
		/*	$('#submit-tampilkan-njop').click(function(){
					if($('#kd-kecamatan').val() != '') {
						$('#data-njop').html("<img src='images/loading.gif' />");
						$("#submit-tampilkan-njop").attr("disabled", "disabled");
						$.post( "modul/bphtb/cari_njop.php", { kd_kecamatan: $('#kd-kecamatan').val(), kd_kelurahan: $('#kd-kelurahan').val()})
  						.done(function( data ) {
   						$('#data-njop').html(data);
							$("#submit-tampilkan-njop").removeAttr('disabled');
  						});
					} else {
