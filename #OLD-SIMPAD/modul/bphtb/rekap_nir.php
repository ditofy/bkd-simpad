...
<button type="button" class="btn btn-success" id="submit-tampilkan-nir">Tampilkan Data NIR</button>
</td>
</tr>
</table>
		<div class="col-md-12 col-sm-3 col-xs-12 bg-white" id="data-nir">		
...
function get_kel(kd) {
	if(kd != ""){
        ...
        $(document).ready(function () {
			...
			$('#submit-tampilkan-nir').click(function(){
					if($('#kd-kecamatan').val() != '') {
						$('#data-nir').html("<img src='images/loading.gif' />");
						$("#submit-tampilkan-nir").attr("disabled", "disabled");
						$.post( "modul/bphtb/cari_nir.php", { kd_kecamatan: $('#kd-kecamatan').val(), kd_kelurahan: $('#kd-kelurahan').val()})
  						.done(function( data ) {
   						$('#data-nir').html(data);
							$("#submit-tampilkan-nir").removeAttr('disabled');
  						});
					} else {
