...
$(document).ready(function () {
	$('#nop').autocomplete({
      			source: "modul/parkir/get_nop.php",
	...
	$('#proses-update-nop-abt').click(function(){
if($('#nop').val() == '' || $('#kd-kecamatan').val() == null || $('#kd-kelurahan').val() == null || $('#kd-keg-usaha').val() == null){
	notif('Notifikasi','Isi Semua Data','error');			
} else {
	$("#proses-update-nop-abt").attr("disabled", "disabled");
	var inputs = $('#frm-update-nop-abt').serializeArray();
					$.post( "modul/parkir/p_mutasi_nop.php", { postdata: inputs })
  						.done(function( data1 ) {
   							notif('Notifikasi',data1,'success');
							$('#resetfrm').click();							
							$("#proses-update-nop-abt").removeAttr('disabled');
  					});
}	
});
	$("#kd-kecamatan").select2("val", "");
...
<form id="frm-update-nop-abt" name="frm-update-nop-abt" data-parsley-validate class="form-horizontal form-label-left">
...
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
		<input type="text" id="kd-obj-pajak" name="kd-obj-pajak" required="required" class="form-control col-md-7 col-xs-12" readonly="readonly" value="07 - parkir">
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
		...
		<?php 
			$sql = "SELECT kd_keg_usaha,nm_keg_usaha FROM public.keg_usaha WHERE kd_obj_pajak='05'";
			$tampil = pg_query($sql) or die('Query failed: ' . pg_last_error());
			while ($row = pg_fetch_array($tampil))
			{			
...
<div class="form-group">
    <div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
       	<button id="proses-update-nop-abt" type="button" class="btn btn-success">Proses</button>
