...
$(document).ready(function () {
  $(".select2_single").select2({
	...
	$("#nik-wp").enterKey(function () {
				//var cek_char = $('#nop').val();
    			//if (cek_char.indexOf('_') == -1) {
				/*	$("#nop").attr("readonly", "readonly"); */
   					$.post( "inc/get_nik_pbb.php", { nik: $('#nik-wp').val() })
  						.done(function( data1 ) {							
							var obj = jQuery.parseJSON(data1);														
							if( obj.status == "ok") {
										$("#nik-wp").val(obj.nik);
										$("#nama-wp").val(obj.nama_wp);
										$("#alamat-wp").val(obj.jalan_wp);
										$("#blok-wp").val(obj.blok_wp);
										$("#rw-wp").val(obj.rw_wp);
										$("#rt-wp").val(obj.rt_wp);
    ...
    return this.each(function () {
        $(this).keypress(function (ev) {
			...
			$("#nop").enterKey(function () {
    			...
    			if (cek_char.indexOf('_') == -1) {
				/*	$("#nop").attr("readonly", "readonly"); */
   					$.post( "inc/get_data_nik_pbb.php", { nop: $('#nop').val() })
  						.done(function( data1 ) {							
							var obj = jQuery.parseJSON(data1);														
							if( obj.status == "ok") {
							            $("#nop").val(obj.nop);
	...
	var inputs = $('#frm-update-pbb-bphtb-nik').serializeArray();
					$.post( "modul/bphtb/p_update_pbb_nik.php", { postdata: inputs,lang:lang })
  						.done(function( data1 ) {
   							notif('Notifikasi',data1,'success');
							$('#resetfrm').click();
							$("#proses-update-pbb-bphtb-nik").removeAttr('disabled');
  					});
...
<form id="frm-update-pbb-bphtb-nik" name="frm-update-pbb-bphtb-nik" data-parsley-validate class="form-horizontal form-label-left">
...
<div class="form-group">
	...
	<div class="form-group">
	 ...
	 <div class="x_panel">
                                <div class="x_title">
                                    <h2>DATA WAJIB PAJAK BARU PBB</h2>   <div class="clearfix"></div>
