...
$sql = "SELECT * FROM reklame.skp WHERE status_pembayaran='0' AND bln_pajak='$bln_pajak' AND thn_pajak='$thn_pajak' ORDER BY kd_kecamatan,kd_kelurahan,kd_obj_pajak,kd_keg_usaha,no_reg";
$tampil = pg_query($sql);
if($tampil) {
//Mempersiapkan
	for($x = 10; $x >= 1; $x--) {
...
//LONG RUNNING TASK
	...
	while ($row = pg_fetch_array($tampil))
	{
		...
		$tmtawalkali=$row['tmt_awal'];
		$tmtakhirkali=$row['tmt_akhir'];
...
//end denda
		$query_cek = "SELECT * FROM reklame.stpd WHERE kd_kecamatan='$kd_kecamatan' AND kd_kelurahan='$kd_kelurahan' AND kd_obj_pajak='$kd_obj_pajak' AND kd_keg_usaha='$kd_keg_usaha' AND no_reg='$no_reg' AND thn_pajak='$thn_pajak' AND bln_pajak='$bln_pajak'";
		...
		if($exec_query_cek) {
			...
			if($ada_data > 0) {
				...
				$query_update = "UPDATE reklame.stpd SET denda=$denda,penyimpan='$nip',tgl_simpan=TO_DATE('$today','DD-MM-YYYY'),tgl_penetapan=TO_DATE('$tgl_penetapan','DD-MM-YYYY'),nip_pejabat='$pejabat' WHERE thn_pajak='$thn_pajak_u' AND bln_pajak='$bln_pajak_u' AND kd_obj_pajak='$kd_obj_pajak' AND no_urut_surat='$no_urut_surat_u'";
				...
				$query = "SELECT TO_NUMBER(MAX(no_urut_surat),'999999') AS no_max FROM reklame.stpd WHERE thn_pajak='$thn_pajak' AND bln_pajak='$bln_pajak'
	AND kd_obj_pajak='$kd_obj_pajak'";
				...
				pg_free_result($result);
				$query_input = "INSERT INTO reklame.stpd(thn_pajak,bln_pajak,no_urut_surat,kd_kecamatan,kd_kelurahan,kd_obj_pajak,kd_keg_usaha,no_reg,kd_provinsi,kd_kota,kd_jns,no_reg_wp,nm_reklame,alamat_reklame,p,l,s,jlh,nm_tarif,tarif,tmt_awal,tmt_akhir,penyimpan,tgl_simpan,status_pembayaran,pokok_pajak,lama_pasang,nip_pejabat,tgl_penetapan,denda) VALUES('$thn_pajak','$bln_pajak','$no_urut_skp','$kd_kecamatan','$kd_kelurahan','$kd_obj_pajak','$kd_keg_usaha','$no_reg','13','76','$kd_jns','$no_reg_wp','$nm_reklame','$alamat_reklame',$p,$l,$s,$jlh,'$nm_tarif',$tarif,'$tmtawalkali','$tmtakhirkali','$nip',TO_DATE('$today','DD-MM-YYYY'),'0',$jlh_skp,$lama_pasang,'$pejabat',TO_DATE('$tgl_penetapan','DD-MM-YYYY'),$denda)";
