...
$num_results = pg_query("SELECT COUNT(*) AS jumlah FROM $schema.skp A INNER JOIN public.wp B ON B.kd_provinsi=A.kd_provinsi AND B.kd_kota=A.kd_kota AND B.kd_jns=A.kd_jns AND B.no_reg=A.no_reg_wp WHERE A.kd_kecamatan LIKE '%$kd_kecamatan%' AND A.kd_kelurahan LIKE '%$kd_kelurahan%' AND A.kd_obj_pajak LIKE '%$kd_obj_pajak%' AND A.kd_keg_usaha LIKE '%$kd_keg_usaha%' AND A.no_reg LIKE '%$no_reg%' AND A.nm_reklame LIKE '%$nm_usaha%' AND A.kd_provinsi LIKE '%$kd_prov%' AND A.kd_kota LIKE '%$kd_kota%' AND A.kd_jns LIKE '%$kd_jns%' AND A.no_reg_wp LIKE '%$no_reg_wp%' AND to_char(A.tmt_awal,'MM') LIKE '%$bln_jtt%' AND to_char(A.tmt_awal,'YYYY') ='$thn_pajak' AND B.nama LIKE '%$nm_wp%' AND A.status_pembayaran='0' AND A.tmt_awal <> '0001-01-01 BC'") or die('Query failed: ' . pg_last_error());
...
$query = pg_query("SELECT A.kd_provinsi||'.'||A.kd_kota||'.'||A.kd_jns||'.'||A.no_reg_wp AS npwpd, A.kd_kecamatan||'.'||A.kd_kelurahan||'.'||A.kd_obj_pajak||'.'||A.kd_keg_usaha||'.'||A.no_reg AS nop,to_char(A.tmt_awal, 'DD-MM-YYYY') AS tmt_aw,to_char(A.tmt_akhir, 'DD-MM-YYYY') AS tmt_ak,A.*,B.* FROM $schema.skp A INNER JOIN public.wp B ON B.kd_provinsi=A.kd_provinsi AND B.kd_kota=A.kd_kota AND B.kd_jns=A.kd_jns AND B.no_reg=A.no_reg_wp WHERE A.kd_kecamatan LIKE '%$kd_kecamatan%' AND A.kd_kelurahan LIKE '%$kd_kelurahan%' AND A.kd_obj_pajak LIKE '%$kd_obj_pajak%' AND A.kd_keg_usaha LIKE '%$kd_keg_usaha%' AND A.no_reg LIKE '%$no_reg%' AND A.nm_reklame LIKE '%$nm_usaha%' AND A.kd_provinsi LIKE '%$kd_prov%' AND A.kd_kota LIKE '%$kd_kota%' AND A.kd_jns LIKE '%$kd_jns%' AND A.no_reg_wp LIKE '%$no_reg_wp%' AND B.nama LIKE '%$nm_wp%' AND to_char(A.tmt_awal,'MM') LIKE '%$bln_jtt%' AND thn_pajak LIKE '%$thn_pajak%' AND A.status_pembayaran='0' AND A.tmt_awal <> '0001-01-01 BC' ORDER BY A.tmt_awal ASC LIMIT $num_per_page OFFSET $offset") or die('Query failed: ' . pg_last_error());
?>
...
$(document).ready(function () {
	$('#cetak-list').click(function(){		
			...
			$.post( "modul/reklame/gen_pdf_list_tagihan.php", { data: inputs, ukuran: $('#ukuran-kertas').val() })
  						.done(function( data1 ) {
	...
	$('#download-xls').click(function(){		
    		...
    		window.open('/print/list_reklame_xls.php');
			});	
	});
...
?>                                        
                                            <tr class="even pointer">
                                                ...
                                                <td class=" "><?php echo $row['nm_reklame']; ?></td>
												<td class=" "><?php echo $row['alamat_reklame']; ?></td>
												<td class=" "><?php echo $row['p']."m x ".$row['l']."m x ".$row['s']." sisi x ".$row['jlh']." buah"; ?></td>
												<td class=" "><?php echo $row['tmt_aw']; ?></td>
