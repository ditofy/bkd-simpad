...
$sql = "SELECT * FROM reklame.dat_obj_pajak WHERE status_pemasangan='PERMANEN' AND status_pajak='1' AND to_char(tmt_akhir,'MM')='12' ORDER BY kd_kecamatan,kd_kelurahan,kd_obj_pajak,kd_keg_usaha,no_reg";
$tampil = pg_query($sql);
if($tampil) {
//Mempersiapkan
	for($x = 10; $x >= 1; $x--) {
...
//LONG RUNNING TASK
	...
	while ($row = pg_fetch_array($tampil))
	{
		...
		$query_cek = "SELECT * FROM reklame.skp WHERE kd_kecamatan='$kd_kecamatan' AND kd_kelurahan='$kd_kelurahan' AND kd_obj_pajak='$kd_obj_pajak' AND kd_keg_usaha='$kd_keg_usaha' AND no_reg='$no_reg' AND thn_pajak='$thn_pajak' AND bln_pajak='12'";
		...
		if($exec_query_cek) {
			...
			/*	$data_update = pg_fetch_array($exec_query_cek);
				...
				$query = "SELECT TO_NUMBER(MAX(no_urut_surat),'999999') AS no_max FROM reklame.skp WHERE thn_pajak='$thn_pajak' AND bln_pajak='$bln_pajak'
