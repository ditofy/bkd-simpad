...
include_once $_SESSION['base_dir']."inc/db.inc.php";
	...
	$sql = "SELECT A.kd_provinsi||'.'||A.kd_kota||'.'||A.kd_jns||'.'||A.no_reg_wp AS npwpd, A.kd_kecamatan||'.'||A.kd_kelurahan||'.'||A.kd_obj_pajak||'.'||A.kd_keg_usaha||'.'||A.no_reg AS nop,A.nm_reklame,A.alamat_reklame,A.p,A.l,A.s,A.jlh,A.id,A.ket,A.jenis_penetapan,A.status_pajak,to_char(A.tgl_daftar, 'DD-MM-YYYY') AS tgl_daf,to_char(A.tmt_awal, 'DD-MM-YYYY') AS tmt_aw,to_char(A.tmt_akhir, 'DD-MM-YYYY') AS tmt_ak,A.kd_tarif,A.lama_pasang,A.status_pemasangan,B.nama,B.alamat,B.telp,B.nik FROM $schema.dat_obj_pajak A
