...
//Mempersiapkan
	for($x = 10; $x >= 1; $x--) {
		send_message('INFO', 'Mempersiapkan Proses Penetapan Masal Surat Teguran '.pg_num_rows($tampil).' Objek Pajak Tahun '.$thn_pajak.' Bulan '.$bulan_s[$bln_pajak].' <br><strong>Jangan Menutup Browser Saat Proses Berjalan</strong><br>Proses Akan Dimulai Dalam Hitungan '.$x, pg_num_rows($tampil)); 
		sleep(1);
	} 
//LONG RUNNING TASK
	$i = 0;
...
//end denda */
		$query_cek = "SELECT * FROM reklame.teguran WHERE jns_surat='$jns_surat' AND thn_pajak='$thn_pajak' AND bln_pajak='$bln_pajak' AND kd_obj_pajak='$kd_obj_pajak' AND no_urut_surat='$no_urut_surat' AND teguran_ke='$teguran_ke'";
	...
	AND kd_obj_pajak='$kd_obj_pajak'";
				...
				pg_free_result($result); */
				$query_input = "INSERT INTO reklame.teguran(jns_surat,thn_pajak,bln_pajak,kd_obj_pajak,no_urut_surat,nip_pejabat,tgl_surat,penyimpan,teguran_ke) VALUES('$jns_surat','$thn_pajak','$bln_pajak','$kd_obj_pajak','$no_urut_surat','$pejabat',TO_DATE('$today','DD-MM-YYYY'),'$nip','$teguran_ke')";
