...
function start_penetapan() {
    ...
    es = new EventSource('/modul/reklame/penetapan_p_masal_pemberitahuan.php?tahun='+$('#tahun-pajak').val()+'&bulan='+$('#bulan-pajak').val()+'&pejabat='+$('#tanda-tangan').val()+'&tanggal_tetap='+$('#tanggal-penetapan').val());
    ...
    es.addEventListener('message', function(e) {
       ...
       // addLog(result.message);       
        ...
        if(e.lastEventId == 'SELESAI') {
			es.close();
            addLog('Selesai Menetapkan Surat Pemberitahuan '+result.progress+' Objek');
			notif('Notifikasi','Penetapan Surat Pemberitahuan Selesai','success');
...
$(document).ready(function () {
	...
	$('#cetak').click(function(){
		$('#modal-cnt').html("<center><br><br><img src='images/loading.gif' /><br>Loading<br><br></center>");		
			$.post( "modul/reklame/gen_pdf_pemberitahuan_masal.php", { thn_pajak: $('#tahun-pajak').val(), bln_pajak: $('#bulan-pajak').val() })
  						.done(function( data1 ) {
...
<div class="form-group">
    	<div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
			...
			<button id="cetak" type="button" class="btn btn-dark" data-toggle="modal" data-target=".bs-example-modal-lg">Cetak Surat Pemberitahuan</button>
		</div>
</div>
</form>
<div class="">
