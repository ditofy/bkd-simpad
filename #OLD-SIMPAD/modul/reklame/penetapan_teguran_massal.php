...
function start_penetapan() {
    ...
    es = new EventSource('/modul/reklame/penetapan_p_teguran_massal.php?tahun='+$('#tahun-pajak').val()+'&bulan='+$('#bulan-pajak').val()+'&pejabat='+$('#tanda-tangan').val()+'&tanggal_tetap='+$('#tanggal-penetapan').val()+'&teguran_ke='+$('#teguran-ke').val());
    ...
    es.addEventListener('message', function(e) {
       ...
       // addLog(result.message);       
        ...
        if(e.lastEventId == 'SELESAI') {
            ...
            $('.progress-bar').css('width', persenp+'%').attr('aria-valuenow', jlhstpd);
	  		$('.progress-bar').html(persenp+'%');
			$("#cetak").removeAttr('disabled');
        } else {
			if(e.lastEventId == 'INFO') {
				jlhstpd = result.progress;
				//$('.progress-bar').attr('aria-valuemax', jlhskp);
				$('.progress-bar').css('width', '0%').attr('aria-valuemax', jlhstpd);
				...
				if(e.lastEventId == 'ERROR') {
					...
					addLog(result.message);
					persenp = (result.progress/jlhstpd)*100;
...
$(document).ready(function () {
	...
	$('#cetak').click(function(){
		$('#modal-cnt').html("<center><br><br><img src='images/loading.gif' /><br>Loading<br><br></center>");		
			$.post( "modul/reklame/gen_pdf_teguran_massal.php", { thn_pajak: $('#tahun-pajak').val(), bln_pajak: $('#bulan-pajak').val() , teguran_ke: $('#teguran-ke').val()})
  						.done(function( data1 ) {
...
<div class="form-group">
	<label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Surat Teguran Ke</label>
    <div class="col-md-6 col-sm-6 col-xs-12">
    	<select name="teguran-ke" id="teguran-ke" class="select2_single" style="width:200px">
