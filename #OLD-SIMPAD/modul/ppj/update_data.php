...
$(document).ready(function () {
				...
				$('#nop').autocomplete({
      			source: "modul/ppj/get_nop.php",
				select: function(event, ui) {
            		if (ui.item.value != 'Data Tidak Ada')  {
					$("#nop").attr("readonly", "readonly");
				$.ajax({
                	type: "GET",
                	url: "modul/ppj/get_detail_objek.php",
			...
			$('#proses-update-ppj').click(function(){
if($('#nop').val() == '' || $('#npwpd').val() == '' || $('#nama-usaha').val() == '' || $('#alamat-usaha').val() == '' || $('#tgl-daftar').val() == '' || $("#status-pajak").val() == null ){
	...
	$("#proses-update-ppj").attr("disabled", "disabled");
	var inputs = $('#frm-update-ppj').serializeArray();
					$.post( "modul/ppj/p_update.php", { postdata: inputs })
  						.done(function( data1 ) {
   							notif('Notifikasi',data1,'success');
							$('#reset').click();
							$("#proses-update-ppj").removeAttr('disabled');
  					});
}	
});
...
<form id="frm-update-ppj" name="frm-update-ppj" data-parsley-validate class="form-horizontal form-label-left">
	...
	<div class="form-group">
    	<div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
        	<button id="proses-update-ppj" type="button" class="btn btn-success">Update</button>
