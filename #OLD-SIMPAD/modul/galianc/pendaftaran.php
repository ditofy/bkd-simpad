...
$(document).ready(function () {
			...
			$('#proses-daftar-galianc').click(function(){
if($('#npwpd').val() == '' || $('#kd-kecamatan').val() == null || $('#kd-kelurahan').val() == null || $('#kd-keg-usaha').val() == null || $('#nama-usaha').val() == '' || $('#alamat-usaha').val() == '' || $('#tgl-daftar').val() == ''){
	...
	$("#proses-daftar-galianc").attr("disabled", "disabled");
	var inputs = $('#frm-daftar-galianc').serializeArray();
					$.post( "modul/galianc/p_tambah.php", { postdata: inputs })
  						.done(function( data1 ) {
   							notif('Notifikasi',data1,'success');
							$('#resetfrm').click();
							$("#proses-daftar-galianc").removeAttr('disabled');
  					});
}	
});
...
<form id="frm-daftar-galianc" name="frm-daftar-galianc" data-parsley-validate class="form-horizontal form-label-left">
	...
	<div class="form-group">
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
        	<input type="text" id="kd-obj-pajak" name="kd-obj-pajak" required="required" class="form-control col-md-7 col-xs-12" readonly="readonly" value="08 - MINERAL DAN BATUAN BUKAN LOGAM">
        </div>
	</div>
	<div class="form-group">
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
			...
			<?php 
			$sql = "SELECT kd_keg_usaha||' - '||nm_keg_usaha AS value FROM public.keg_usaha WHERE kd_obj_pajak='08'";
			$tampil = pg_query($sql) or die('Query failed: ' . pg_last_error());
			while ($row = pg_fetch_array($tampil))
			{
	...
	<div class="form-group">
    	<div class="col-md-9 col-sm-9 col-xs-12 col-md-offset-3">
        	<button id="proses-daftar-galianc" type="button" class="btn btn-success">Simpan</button>
