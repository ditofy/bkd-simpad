...
$query = pg_query("SELECT A.kd_provinsi||'.'||A.kd_kota||'.'||A.kd_jns||'.'||A.no_reg_wp AS npwpd, A.kd_kecamatan||'.'||A.kd_kelurahan||'.'||A.kd_obj_pajak||'.'||A.kd_keg_usaha||'.'||A.no_reg AS nop,to_char(A.tgl_daftar, 'DD-MM-YYYY') AS tgl_daf,A.*,B.* FROM $schema.dat_obj_pajak A INNER JOIN public.wp B ON B.kd_provinsi=A.kd_provinsi AND B.kd_kota=A.kd_kota AND B.kd_jns=A.kd_jns AND B.no_reg=A.no_reg_wp WHERE A.kd_kecamatan LIKE '%$kd_kecamatan%' AND A.kd_kelurahan LIKE '%$kd_kelurahan%' AND A.kd_obj_pajak LIKE '%$kd_obj_pajak%' AND A.kd_keg_usaha LIKE '%$kd_keg_usaha%' AND A.no_reg LIKE '%$no_reg%' AND A.nm_usaha LIKE '%$nm_usaha%' AND A.kd_provinsi LIKE '%$kd_prov%' AND A.kd_kota LIKE '%$kd_kota%' AND A.kd_jns LIKE '%$kd_jns%' AND A.no_reg_wp LIKE '%$no_reg_wp%' AND B.nama LIKE '%$nm_wp%' ORDER BY A.id LIMIT $num_per_page OFFSET $offset") or die('Query failed: ' . pg_last_error());
?>
...
$(document).ready(function () {
	$('#cetak-list').click(function(){		
			...
			$.post( "modul/galianc/gen_pdf_list.php", { data: inputs, ukuran: $('#ukuran-kertas').val() })
  						.done(function( data1 ) {
	...
	$('#download-xls').click(function(){		
    		...
    		window.open('/print/list_galianc_xls.php');
			});	
	});
