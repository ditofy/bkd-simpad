...
$awal = "1/1/".date('Y');
$akhir = date('d/m/Y');
$query = "SELECT SUM(pokok_pajak) AS r_bphtb FROM bphtb.pembayaran WHERE tgl_bayar BETWEEN TO_DATE('$awal','DD-MM-YYYY') AND TO_DATE('$akhir','DD-MM-YYYY')";
$result = pg_query($query) or die('Query failed: ' . pg_last_error());
$data = pg_fetch_array($result);
$pokok = $data['r_bphtb'];
$query = "SELECT target FROM public.target WHERE thn_pajak='$tahun_pajak' AND kd_obj_pajak='09'";
$result = pg_query($query) or die('Query failed: ' . pg_last_error());
$data = pg_fetch_array($result);
$target = $data['target'];
