...
$query = pg_query("SELECT A.kd_provinsi||'.'||A.kd_kota||'.'||A.kd_jns||'.'||A.no_reg AS npwpd,A.* FROM $schema.wp A WHERE A.kd_provinsi LIKE '%$kd_prov%' AND A.kd_kota LIKE '%$kd_kota%' AND A.kd_jns LIKE '%$kd_jns%' AND A.no_reg LIKE '%$no_reg_wp%' AND A.nama LIKE '%$nm_wp%'  AND A.status <> '0' ORDER BY A.kd_provinsi,A.kd_kota,A.kd_jns,A.no_reg") or die('Query failed: ' . pg_last_error());
?>
<style type="text/css">
